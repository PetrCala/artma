...
- `winsorize`: `FALSE` hands the method the data frame prepared without winsorization (`effect`, `se`, and the columns derived from them unclipped; everything else identical) instead of the shared winsorized frame. The orchestrator builds that frame lazily from the run context (`unwinsorized_df` in `prepare_run_context()`), so a direct `invoke_runtime_methods()` call that supplies none falls back to `df` with a warning when winsorization is active. Use it for methods that winsorization distorts, such as `p_hacking_tests`, whose p-curve density tests read clipping as the bunching they detect. Building that frame reruns the data pipeline with the clipping off, so it can abort where the winsorized pass did not (a zero standard error winsorization lifted off zero, under `calc.se_zero_handling: "stop"`); the failure is reported against the methods that asked for the frame and the rest of the run continues.
