...
`exogeneity_tests` needs the `AER` package and `maive` needs the `MAIVE` package (>= 0.2.4); the others in this group work with the package's own dependencies.
...
| `robma` | Robust Bayesian meta-analysis (model-averaged publication-bias correction) via the `RoBMA` package; opt-in, run it by requesting it by name | none | `effect`, `se` |
